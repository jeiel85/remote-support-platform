...
public sealed class GeneratedContractTests
{
    ...
    {
        Assert.Equal(48, OpenApiContract.Operations.Count);
        Assert.Equal(48, OpenApiContract.Operations.Select(operation => operation.OperationId).Distinct(StringComparer.Ordinal).Count());
    }
